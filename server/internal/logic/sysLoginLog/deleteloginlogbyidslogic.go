...
func (l *DeleteLoginLogByIdsLogic) DeleteLoginLogByIds(req *types.DeleteLoginLogByIdsReq) (resp *types.Response, err error) {
	...
	// 2. 批量软删除
	if err = l.svcCtx.DB.WithContext(l.ctx).Where("id in ?", req.Ids).Delete(&model.SysLoginLog{}).Error; err != nil {
		l.Logger.Errorw("批量删除登录日志失败",
			logx.Field("error", err),
			logx.Field("ids", req.Ids),
			logx.Field("module", "sysLoginLog"),
			logx.Field("action", "delete_by_ids"),
		)
		return nil, errors.Wrap(err, "批量删除登录日志失败")
	}
